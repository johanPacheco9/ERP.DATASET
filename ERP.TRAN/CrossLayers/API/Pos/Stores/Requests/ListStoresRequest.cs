...
public sealed class ListStoresRequest : BaseListRequest, IValidatableRequest
{
    /// <summary>
    /// Crea una solicitud para listar las tiendas, se ordena dependiendo del parametro especificado.
    /// </summary>
    ...
    {
        OrderBy = $"{nameof(StoreSummaryDto.Id)}";
    }
    ...
    public ListStoresRequest(int pageNumber, int pageSize,
    ...
    public override bool ParametersAreValid(out string? errors)
    {
        ...
        if (!string.IsNullOrWhiteSpace(OrderBy))
        {
            var validFields = new[]
            {
                nameof(StoreSummaryDto.Id),
                nameof(StoreSummaryDto.Name)
            };
