...
public class ListUsersRequest : BaseListRequest
{
    ...
    public override bool ParametersAreValid(out string? errors)
    {
        ...
        if (PageSize == 0)
            errorList.Add(
                "El tamaño de página no puede ser 0. Usa -1 si deseas obtener todos los registros sin límite.");
        ...
        if (!string.IsNullOrWhiteSpace(OrderBy))
        {
            ...
            {
                errorList.Add(
                    $"El campo de ordenamiento '{OrderBy}' no es válido. Campos permitidos: {string.Join(", ", validFields)}.");
            }
        }
