...
public partial class Update
{
    ...
    [Inject] public WarehouseService WarehouseManager { get; set; } = null!;
    ...
    private async Task LoadStoreDataAsync()
    {
        try
        {
            ...
            var cajas = await CajaManager.List(store.Id);
            ...
            var warehouseRequest = new ListWarehousesRequest
            {
                StoreId = store.Id,
                PageNumber = 1,
            };

            var warehouseResult = await WarehouseManager.List(warehouseRequest, default);
            _bodegas = warehouseResult;
        }
        catch (Exception ex)
        {
