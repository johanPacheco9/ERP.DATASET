...
public partial class CrearProductoBase : ComponentBase
{
    ...
    private List<MarcasDetailDto> _marcas = [];
    ...
    private async Task GetMarcas()
    {
        try
        {
            var result = await MarcasManager.List();
            if (result.IsSuccess)
            {
                _marcas = result.Value;
            }
            else
            {
                _errores.Add($"Error consultando las marcas");
            }
        }
        catch (Exception exception)
        {
            _errores.Add($"Error inicializando: {exception.Message}");
        }
    }

    private void ToggleCategoria(int categoriaId, object? value)
    {
        if (value is bool isChecked)
        {
            if (isChecked)
            {
                if (!_form.CategoriaIds.Contains(categoriaId))
                    _form.CategoriaIds.Add(categoriaId);
            }
            else
            {
                _form.CategoriaIds.Remove(categoriaId);
            }
        }
    }
    ...
    private async Task CrearProducto()
    {
        ...
        {
            _errores.Add("El código es obligatorio.");
            return;
        }

        if (string.IsNullOrWhiteSpace(_form.Nombre))
        {
            _errores.Add("El nombre es obligatorio.");
            return;
        }

        if (_form.CategoriaIds == null || !_form.CategoriaIds.Any())
        {
            _errores.Add("Debe seleccionar al menos una categoría.");
            return;
        }

        if (_form.MarcaId <= 0)
        {
            _errores.Add("Debe seleccionar una marca.");
            return;
        }

        if (_form.Costo_Unitario <= 0)
        {
            _errores.Add("El costo unitario de referencia debe ser mayor a cero.");
            return;
        }

        if (_form.Precio_Venta <= 0)
        {
            _errores.Add("El precio de venta debe ser mayor a cero.");
            return;
        }
        ...
        try
        {
            ...
            {
                _errores.Add("No se pudo crear el producto. Intente nuevamente.");
            }
        }
        catch (Exception ex)
        {
