...
{
    public async Task<Result<BarcodeLookupResultDto?>> LookupProductByBarcodeAsync(
        string code,
        int warehouseId,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(code))
            return Result<BarcodeLookupResultDto?>.Success(null);
        ...
        var unidadFisica = await context.UnidadesProductos
            ...
            .FirstOrDefaultAsync(u => u.BodegaId == warehouseId &&
                                      u.SerialNumber == cleanCode, cancellationToken);

        if (unidadFisica != null && unidadFisica.Status == UnidadProductoStatus.InAuditLock)
        {
            return Result<BarcodeLookupResultDto?>.Failure(
                new Error("Product.Blocked", "El producto está bloqueado por una auditoría."));
        }

        return await Result<BarcodeLookupResultDto?>.TryAsync(async () =>
            {
                if (unidadFisica != null)
                {
                    ...
                    return new BarcodeLookupResultDto(
                        ...
                        productoBase.Categorias?.Select(s => new CategoriaDetailDto(s.Category.Id, s.Category.Name,
                            s.Category.Description, s.Category.CreatedAt, s.Category.UpdatedAt)).ToList()
                    );
                }
                ...
                if (varianteMatch != null)
                {
                    ...
                    return new BarcodeLookupResultDto(
                        ...
                        productoBase.Categorias?.Select(s => new CategoriaDetailDto(s.Category.Id, s.Category.Name,
                            s.Category.Description, s.Category.CreatedAt, s.Category.UpdatedAt)).ToList() // Categoria
                    );
                }

                return null;
            },
            ex => new Error("Product.LookupFailed", ex.Message));
    }

    public async Task<Result<List<BarcodeLookupResultDto>>> SearchProductsForPosAsync(
        string? query,
        ...
        CancellationToken cancellationToken = default)
    {
        return await Result<List<BarcodeLookupResultDto>>.TryAsync(async () =>
            {
                ...
                if (categoryIds != null && categoryIds.Count > 0)
                {
                    variantesQuery = variantesQuery.Where(v =>
                        v.ProductoBase.Categorias.Any(ct => categoryIds.Contains(ct.CategoryId)));
                }
                ...
                foreach (var v in variantes)
                {
                    ...
                    result.Add(new BarcodeLookupResultDto(
                        ...
                        b.ImagenUrl,
                        b.UnidadMedida ?? "UND",
                        b.Categorias?.Select(s => new CategoriaDetailDto(s.Category.Id, s.Category.Name,
                            s.Category.Description, s.Category.CreatedAt, s.Category.UpdatedAt)).ToList()
                    ));
                }
            ...
            ex => new Error("Pos.SearchProductsFailed", ex.Message));
    }
}